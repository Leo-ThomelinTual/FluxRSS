...
<template name="frame">
  <section
    class="border-2 rounded-md border-opacity-25 hover:bg-gray-500 hover:bg-opacity-10 border-gray-500 w-full h-[100%] overflow-y-auto p-4 hover:p-5 hover:border-opacity-100 transition-all duration-300 ease"
  >
    <header class="mb-6">
      <h1 class="text-xl font-bold mb-2"><slot name="flux-title"></slot></h1>
    </header>

    <div v-if="loading" class="text-center text-gray-500">Chargement...</div>
    <div v-else-if="error" class="text-red-500">{{ error }}</div>
    <section v-else class="feed-content space-y-4 overflow-y-auto">
      <article
